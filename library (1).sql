...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Фев 15 2019 г., 10:23
...
ALTER TABLE `books`
  ...
  ADD KEY `publishing_company_id` (`publishing_company_id`);
...
ALTER TABLE `books`
  ...
  ADD CONSTRAINT `books_ibfk_2` FOREIGN KEY (`publishing_company_id`) REFERENCES `publishings` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Ограничения внешнего ключа таблицы `books_issued`
--
ALTER TABLE `books_issued`
  ADD CONSTRAINT `books_issued_ibfk_1` FOREIGN KEY (`student_id`) REFERENCES `student` (`id`),
  ADD CONSTRAINT `books_issued_ibfk_2` FOREIGN KEY (`book_id`) REFERENCES `books` (`id`) ON DELETE NO ACTION;
COMMIT;
